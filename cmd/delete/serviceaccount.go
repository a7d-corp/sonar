...
func deleteServiceAccount(k8sClientSet *kubernetes.Clientset, ctx context.Context, o config.DeleteConfig, force bool) (string, err) {
	...
	if ok {
		err = k8sClientSet.CoreV1().ServiceAccounts(o.Namespace).Delete(ctx, o.Name, deleteOptions)
	}
	...
	if statusError, isStatus := err.(*errors.StatusError); isStatus && statusError.Status().Reason == metav1.StatusReasonNotFound {
		...
		return "", fmt.Errorf("serviceaccount \"%s/%s\" failed deletion: %w", opts.Namespace, opts.Name, err)
	} else {
