...
func DeleteClusterRoleBinding(k8sClientSet *kubernetes.Clientset, ctx context.Context, sonarConfig clientconfigs.SonarConfig, force bool) (err error) {
	...
	if ok {
		err = k8sClientSet.RbacV1().ClusterRoleBindings().Delete(ctx, sonarConfig.Name, deleteOptions)
	}
